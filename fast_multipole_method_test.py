...
class operation:
    ...
    def M2L(Mlm_x1, X12):
        ...
        p = Mlm_x1.degree
        R12 = operation.cartesian_to_spherical(X12)
        Tlm_X12 = operation.L_expansion(p*2, R12)
        ...
        for j in range(0, p+1):
            for k in range(-j, j+1):
                ...
                Tlm_X12_subm = Tlm_X12.get_sub_matrix(Tlm_lr, Tlm_mr)
                products = Tlm_X12_subm * Mlm_x1.V_matrix * np.power(1j, m_power) \
    ...
    def L2L(Llm_x1, X21):
        ...
        """
        p = int(Llm_x1.degree / 2)
        R21 = operation.cartesian_to_spherical(X21)
        Tlm_X21 = operation.M_expansion(p, R21)
        Tlm_m_power = operation.m_power(p)

        Llm_m_power = operation.m_power(p*2)
        Llm_l_power = operation.l_power(p*2)
