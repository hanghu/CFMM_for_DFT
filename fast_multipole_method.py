...
class operation:
    ...
    def L2L(Llm_x1, X21):
        ...
        Tlm_X21 = operation.M_expansion(p*2, R21)
        ...
        for j in range(0, p+1):
            for k in range(-j, j+1):
                ...
                Tlm_lr = [j, j+p]; Tlm_mr = [k-p, k+p]
                Tlm_m_power_subm = Tlm_m_power.get_sub_matrix(Tlm_lr, Tlm_mr)
                m_power = Llm_m_power.V_matrix - Tlm_m_power_subm - np.abs(k)
                l_power = Llm_l_power.V_matrix + j

                Tlm_X21_subm = Tlm_X21.get_sub_matrix(Tlm_lr, Tlm_mr)
                products = Tlm_X21_subm * Llm_x1.V_matrix * np.power(1j, m_power) \
                    * np.power(-1, l_power)
