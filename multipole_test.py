...
class operation:
    ...
    def Olm_X12_to_X21(Olm):
        p = Olm.p
        for l in range(0, p+1):
            if l%2:
                for m in range(-l, l+1):
                    Olm.setlm(l, m, -Olm.getlm(l, m).conjugate())
            else:
                for m in range(-l, l+1):
                    Olm.setlm(l, m, Olm.getlm(l, m).conjugate())

    def O_to_M(Ojk_x1_k, X21):
        """
        ...
        Clmjk_X21 = operation.M_expansion(p*2, R21)
        operation.Olm_X12_to_X21(Ojk_x1_k)
        Mlm_x2_k = Vlm(p)
        for l in range(0, p+1):
            for m in range(-l, l+1):
                temp = 0
    ...
    def M_to_M(Mlm_x1_k, X12):
        ...
        Tjklm_X12 = operation.O_expansion(p, R12)
        Mjk_x2_k = Vlm(p)
